...
var (
	...
	bootcmd  = flag.String("bootcmd", "boot", "command run (mirrored) before the shell; empty to go straight to the shell")
)
...
func main() {
	...
	// Reproduce the normal LinuxBoot flow on the mirrored terminal: run the
	// bootloader first (now mirrored to every console), and fall back to an
	// interactive shell if it returns instead of kexec'ing into a target.
	// Anything passed as positional args overrides this and is run verbatim.
	var script string
	switch {
	case len(flag.Args()) > 0:
		script = strings.Join(flag.Args(), " ")
	case *bootcmd != "":
		script = fmt.Sprintf("while true; do %s; %s; done", *bootcmd, *shell)
	default:
		script = *shell
	}
